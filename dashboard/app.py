...
@login_required
def usb_flash_log():
    """Log USB flash session from ESP Web Tools
    
    Creates a complete flash log entry with esptool output captured from the browser.
    Expected payload:
    {
        "action": "complete",
        "platform": "esp32-s3",
        ...
        "log_content": "full esptool output..."
    }
    """
    from models import OTAUpdateLog
    import uuid
    ...
    if action != 'complete':
        return jsonify({'error': 'Invalid action. Use complete'}), 400
    ...
    print(f"{status_emoji} USB flash {log.status}: {product}/{platform} v{version} ({log_lines} log lines){error_info}")
    
    return jsonify({
        'success': True,
        'session_id': session_id,
        'message': f'USB flash logged as {log.status}'
    })
