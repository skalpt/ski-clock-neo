...
*   **Environment Scope:** Devices have an environment field (`dev` or `prod`) that determines their MQTT topic namespace. The environment scope MUST be set at compile-time via `ENV_SCOPE` flag (no default - build fails if not specified). This can be changed at runtime via MQTT config command `{"environment": "prod"}`. Changes are persisted to NVS (ESP32) or EEPROM (ESP8266) and trigger automatic MQTT reconnection to new topic namespace.
...
*   **Device Management:** Integrates with PostgreSQL for persistent device tracking, displaying status, environment scope, and allowing deletion. Monitors device heartbeat timestamps for "online," "degraded," and "offline" statuses. Devices include `environment` field indicating whether they are in `dev` or `prod` namespace.
...
*   **Dev/Production Environment Separation:** Production acts as stable proxy for development. Dev environment auto-registers with production on startup using `PRODUCTION_API_URL` and maintains connection via 60-second heartbeats. Production provides `/api/dev/register`, `/api/dev/heartbeat`, `/api/dev/status`, `/api/dev/upload`, and `/api/dev/config` endpoints for proxy functionality. Dashboard header displays DEV/PROD badge based on `is_production()` detection. GitHub Actions: `build-firmware.yml` (auto-triggers on push to main, ENV_SCOPE=dev, saves version manifest for promotion) and `promote-prod.yml` (manual trigger, fetches version from last successful dev build, ENV_SCOPE=prod). This ensures version numbers are preserved across environments for traceability. Legacy `deploy-prod.yml` is deprecated. Note: `deploy-dev.yml` should be deleted from GitHub as it's a duplicate.
