...
*   **MQTT Integration**: Publishes device heartbeats (ID, firmware version, uptime, RSSI, etc.) to HiveMQ Cloud every 60 seconds. The dashboard automatically checks for new firmware versions and sends update notifications. Supports TLS encryption and increased buffer size for display snapshots.
