...
2.  **Dashboard Server**: A Python Flask application for firmware distribution and device management.
    ...
    *   **Authentication**: Session-based authentication protects dashboard routes (`/`, `/api/status`, `/api/devices`) requiring login credentials stored in Replit Secrets (`DASHBOARD_USERNAME`, `DASHBOARD_PASSWORD`). Device API routes (`/api/firmware/<platform>`, `/api/version`, `/api/upload`) use API key authentication. Public firmware downloads use signed tokens.
    *   **Public Firmware Downloads**: Uses signed URL tokens (itsdangerous URLSafeTimedSerializer) for secure public firmware downloads without exposing API keys. Tokens expire after 24 hours. Dashboard users access firmware via `/firmware/download/<token>` while devices use authenticated `/api/firmware/<platform>` endpoint.
