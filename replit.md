...
1.  **Firmware**: Embedded C++ for ESP32/ESP8266 microcontrollers driving a 16x16 NeoPixel LED matrix.
    ...
    *   **MQTT Command Handling**: Subscribes to device-specific command topic `skiclock/command/<device_id>` for remote management. Supports `rollback` command (ESP32: instant rollback via dual OTA partition switching using `esp_ota_set_boot_partition`; ESP8266: re-downloads previous firmware version due to single-partition limitation) and `restart` command (immediate device reboot). JSON command parsing with graceful error handling.
    *   **OTA Progress Reporting**: Publishes real-time OTA update status to MQTT topics: `skiclock/ota/start` (device ID, platform, old/new versions), `skiclock/ota/progress` (download percentage reported every 10%), and `skiclock/ota/complete` (success/failure status with error messages). Chunked download with incremental Update.write() enables granular progress tracking. Dashboard receives live updates for progress bars and completion status.
...
2.  **Dashboard Server**: A Python Flask application for firmware distribution and device management.
    ...
    *   **OTA Update Logging & Monitoring**: Comprehensive OTA update tracking via PostgreSQL OTAUpdateLog table with session-based tracking. MQTT topics (`skiclock/ota/start`, `skiclock/ota/progress`, `skiclock/ota/complete`) log every OTA attempt with device ID, platform, old/new versions, progress percentage, and success/failure status. Session IDs (UUIDs) enable tracking individual update attempts. Backward-compatible fallback logic matches updates by device_id when session_id is absent. Dashboard displays real-time OTA statistics: total updates, success rate, failed count, in-progress updates, average duration, and recent activity. API endpoint `/api/ota-logs` supports advanced filtering (device_id, status, date range with ISO 8601 timestamps, pagination with limit/offset) for time-bounded queries. `/api/ota-stats` provides aggregated metrics. Dedicated OTA history page (`/ota-history`) provides comprehensive filtering UI with device, status, and date range filters, pagination, real-time progress bars, and auto-refresh every 30 seconds. Device cards on main dashboard show live OTA progress with animated progress bars during firmware updates via `/api/ota-progress` endpoint.
    *   **MQTT Command System**: Dashboard can send remote commands to devices via MQTT topic `skiclock/command/<device_id>`. Commands include `rollback` (restore previous firmware version) and `restart` (reboot device). API endpoints `/api/devices/<device_id>/rollback` and `/api/devices/<device_id>/restart` publish commands with QoS 1 delivery. Device cards include rollback and restart action buttons for easy fleet management. Command payloads are JSON with command type, timestamp, and optional parameters (e.g., target_version for rollback). MQTT client maintains global reference for command publishing.
