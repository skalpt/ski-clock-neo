...
*   **MQTT Integration:** Uses a split messaging architecture with two topic types: `norrtek-iot/heartbeat/{device_id}` for dynamic telemetry (rssi, uptime, free_heap, ssid, ip) published every 60s, and `norrtek-iot/info/{device_id}` for static device info (product, board, version, config, supported_commands) published on connect, after config changes, and on-demand via 'info' command. Supports TLS encryption. Subscribes to device-specific topics for remote `rollback`, `restart`, `snapshot`, `info`, and `config` commands.
*   **Remote Configuration:** Devices subscribe to `norrtek-iot/config/{device_id}` for runtime configuration updates. Temperature offset (-20°C to +20°C) is configurable via MQTT and persisted to NVS (ESP32) or EEPROM (ESP8266). Configuration changes emit events (config_updated, config_error, config_noop) for dashboard visibility.
