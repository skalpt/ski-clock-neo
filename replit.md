...
*   **Strict Dev/Production Environment Separation:** Each dashboard deployment (dev or prod) operates in complete isolation. MQTT subscriptions, API endpoints, and data queries are all scoped to the dashboard's own environment. All device management endpoints (delete, rollback, restart, snapshot, config, rename, pin-version) verify the device belongs to the current environment. All data endpoints (events, snapshots, OTA logs, OTA stats, OTA progress, commands) filter results to only show data from devices in the current environment. Cross-environment access is prevented at all levels. Production acts as stable proxy for development for firmware sync. Dev environment auto-registers with production on startup using `PRODUCTION_API_URL` and maintains connection via 60-second heartbeats. Dashboard header displays DEV/PROD badge based on `is_production()` detection. GitHub Actions: `build-firmware.yml` (auto-triggers on push to main, saves version manifest for promotion) and `promote-prod.yml` (manual trigger, fetches version from last successful dev build). Device environment scope is managed at runtime via MQTT, not at compile time.
