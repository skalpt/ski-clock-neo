...
*   **Dev/Production Environment Separation:** Production acts as stable proxy for development. Dev environment auto-registers with production on startup using `PRODUCTION_API_URL` and maintains connection via 60-second heartbeats. Production provides `/api/dev/register`, `/api/dev/heartbeat`, `/api/dev/status`, `/api/dev/upload`, and `/api/dev/config` endpoints for proxy functionality. Dashboard header displays DEV/PROD badge based on `is_production()` detection. GitHub Actions: `deploy-dev.yml` (auto-triggers on push to main, ENV_SCOPE=dev, saves version manifest) and `promote-prod.yml` (manual trigger, fetches version from last successful dev build, ENV_SCOPE=prod). This ensures version numbers are preserved across environments for traceability. Legacy `build-firmware.yml` and `deploy-prod.yml` are deprecated.
