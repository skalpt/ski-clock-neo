...
*   **Modular Display Architecture**: Generic `display.{h,cpp}` library manages display buffer (bit-packed for memory efficiency) and content generation (digit counter with 5x7 font), decoupled from rendering hardware. The `neopixel_render.h` reads from display buffer and renders to NeoPixel hardware, enabling future migration to HUB75 panels without changing display logic.
*   **MQTT Integration**: Publishes device heartbeats (ID, board type, firmware version, uptime, WiFi RSSI, free heap, SSID, IP) to HiveMQ Cloud. Manages version checking by requesting updates on boot and hourly, receiving device-specific responses, and triggering OTA. Subscribes to broadcast `skiclock/version/updates` for real-time notifications. Supports TLS encryption without certificate validation. MQTT buffer increased to 2048 bytes to support display snapshots.
