...
*   **Environment Scope:** Devices have an environment field (`dev` or `prod`) that determines their MQTT topic namespace. Default is "dev" - devices start in development mode and must be explicitly promoted to production via MQTT config command `{"environment": "prod"}`. Environment is stored as a single-byte enum in EEPROM/NVS (0=default/dev, 1=dev, 2=prod) for efficiency. Changes trigger automatic MQTT reconnection to the new topic namespace.
