...
*   **MQTT Integration**: Publishes device heartbeats (ID, board type, firmware version, uptime, WiFi RSSI, free heap, SSID, IP) to HiveMQ Cloud. Receives automatic version updates on every heartbeat via `skiclock/version/response/{device_id}` topic. Subscribes to broadcast `skiclock/version/updates` for real-time notifications when new firmware is uploaded. Supports TLS encryption without certificate validation. MQTT buffer increased to 2048 bytes to support display snapshots. JSON parsing handles both standard (with spaces) and compact (without spaces) formats for maximum compatibility.
*   **Display Snapshot System**: Publishes hourly snapshots of display state to `skiclock/display/snapshot/{device_id}` with base64-encoded bit-packed pixel data, panel dimensions (rows/cols), and resolution (width/height). Supports on-demand snapshots via `snapshot` MQTT command for real-time troubleshooting.
...
*   **MQTT Integration**: Background subscriber connects to HiveMQ Cloud, processes heartbeats and version requests, persists device data to PostgreSQL, and exposes live device data via REST API. **Automatic version checking**: On every heartbeat (every 60s), dashboard compares device version with latest available firmware using authoritative BOARD_TYPE_TO_PLATFORM mapping and automatically publishes update notification to `skiclock/version/response/{device_id}` if newer version exists. All 6 board types (ESP32, ESP32-C3, ESP32-S3, ESP-12F, ESP-01, Wemos D1 Mini) are correctly mapped to their firmware platform identifiers. **Broadcast updates**: When new firmware is uploaded, dashboard broadcasts to `skiclock/version/updates` topic to notify all listening devices. Subscribes to `skiclock/display/snapshot/#` for display state monitoring.
*   **Display Snapshot Visualization**: Stores display snapshots in PostgreSQL (JSON column with base64 pixel data and dimensions), provides API endpoint (`/api/devices/<id>/snapshot`) to request on-demand snapshots, and enables visual rendering of device display state for remote troubleshooting.
